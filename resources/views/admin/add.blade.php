...
<div class="d-flex" id="wrapper">
  ...
  <div id="page-content-wrapper" class="site">
    ...
    <div class="content">
      ...
      <h4 class="font-weight-bold my-3 mt-md-4">Tambah Prestasi</h4>

      <form action="{{ url('/add/send') }}" method="post" enctype="multipart/form-data" class="add">
        @csrf
              ...
              <div class="col-md-6">
                <div class="form-group input-group-sm">
                  ...
                  <input type="text" class="form-control @error('team') is-invalid @enderror"
                         id="team" name="team" value="{{ old('team') }}" placeholder="Masukkan Nama Tim">
                ...
                <div class="form-group input-group-sm">
                  ...
                  <select id="team_type" name="team_type" class="form-control custom-select @error('team_type') is-invalid @enderror" required>
                    <option value="" disabled {{ old('team_type') ? '' : 'selected' }}>Pilih tipe</option>
                    <option value="Individu" {{ old('team_type')==='Individu' ? 'selected':'' }}>Individu</option>
                    <option value="Kelompok" {{ old('team_type')==='Kelompok' ? 'selected':'' }}>Kelompok</option>
                  </select>
                ...
                <div class="form-group input-group-sm">
                  ...
                  <select class="form-control custom-select @error('field') is-invalid @enderror" id="field" name="field">
                    <option value="" disabled {{ old('field') ? '' : 'selected' }}>Pilih Bidang</option>
                    <option value="Akademik"    {{ old('field')==='Akademik' ? 'selected':'' }}>Akademik</option>
                    <option value="NonAkademik" {{ old('field')==='NonAkademik' ? 'selected':'' }}>Non-Akademik</option>
                  </select>
                ...
                <div class="form-group input-group-sm">
                  ...
                  <select class="form-control custom-select @error('level') is-invalid @enderror" id="level" name="level">
                    <option value="" disabled {{ old('level') ? '' : 'selected' }}>Pilih Tingkat</option>
                    <option value="Region"        {{ old('level')==='Region' ? 'selected':'' }}>Regional</option>
                ...
                <div class="form-group input-group-sm">
                  ...
                  <input type="text" class="form-control @error('organizer') is-invalid @enderror"
                         id="organizer" name="organizer" value="{{ old('organizer') }}" placeholder="Nama penyelenggara">
              ...
              <div class="col-md-6">
                <div class="form-group input-group-sm">
                  ...
                  <input type="text" class="form-control @error('competition') is-invalid @enderror"
                         id="competition" name="competition" value="{{ old('competition') }}" placeholder="Nama kompetisi">
                  @error('competition')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="form-group input-group-sm">
                  <label for="achievement">Pencapaian <span class="text-danger">*</span></label>
                  <input type="text" class="form-control @error('achievement') is-invalid @enderror"
                         id="achievement" name="achievement" value="{{ old('achievement') }}" placeholder="Contoh: Juara 1 / Peserta">
                ...
                <div class="form-group input-group-sm">
                  ...
                  <input type="text" class="form-control @error('rank') is-invalid @enderror"
                         id="rank" name="rank" value="{{ old('rank') }}" placeholder="Contoh: Juara 1 / Harapan 1">
                  ...
                  <div class="col-md-6">
                    <div class="form-group input-group-sm">
                      ...
                      <select class="form-control custom-select @error('month') is-invalid @enderror" id="month" name="month">
                        <option value="" disabled {{ old('month') ? '' : 'selected' }}>Pilih Bulan</option>
                        @for($m=1;$m<=12;$m++)
                          @php $val = $m < 10 ? '0'.$m : (string)$m; @endphp
                          <option value="{{ $val }}" {{ (string)old('month')===$val ? 'selected' : '' }}>
                  ...
                  <div class="col-md-6">
                    <div class="form-group input-group-sm">
                      ...
                      <input type="number" class="form-control @error('year') is-invalid @enderror"
                             id="year" name="year" value="{{ old('year') }}" placeholder="YYYY" min="1900" max="{{ date('Y') + 1 }}">
                ...
                <div class="form-group input-group-sm">
                  <label for="documents">Dokumentasi (opsional)</label>
        ...
        <small class="text-muted d-block mb-3">
          Urutan <b>sertifikat</b> harus sesuai dengan urutan <b>peserta</b>.
        </small>

        <div class="row mt-3">
          <div class="col-md"><i><span class="text-danger">*</span> <small>Wajib diisi</small></i></div>
          <div class="col-md text-md-right"><button type="submit" class="btn btn-md btn-primary">Simpan</button></div>
        </div>
      </form>
    </div>
...
{{-- TEMPLATE 1 KARTU PESERTA (hidden) --}}
...
<script>
  ...
  function wireRemove(scope) {
    ...
    for (var i=0;i<btns.length;i++){
      btns[i].onclick = function(){
        ...
        if (type === 'Individu') return; // tidak bisa hapus
        var min = 2;
        var cols = wrap.querySelectorAll('.participant-col');
        if (cols.length <= min) return;
  ...
  function renderParticipantsForType(type, keepExistingOld=false) {
    // hitung default jumlah
    var defaultCount = (type === 'Kelompok') ? 2 : 1;
    ...
    setGrid(type === 'Kelompok');
    toggleControlsByType(type);
  }
  ...
  teamTypeEl.addEventListener('change', function(){
    renderParticipantsForType(this.value);
  });
  ...
  (function init(){
    ...
    var oldType = @json(old('team_type'));
    var oldNames = @json(old('names', []));
    var initialType = oldType || 'Individu';

    // Set selected type if not auto-selected
    if (!oldType && teamTypeEl.querySelector('option[value="Individu"]')) {
      teamTypeEl.value = 'Individu';
    }

    // Jika ada old('names'), render sesuai jumlah (min 1 / min 2 by type)
    if (oldNames && oldNames.length) {
      // kosongkan lalu rebuild sesuai old length
      wrap.innerHTML = '';
      var minReq = (initialType === 'Kelompok') ? 2 : 1;
      var count = Math.max(minReq, oldNames.length);
      for (var i=0;i<count;i++){
        var card = makeCard();
        wrap.appendChild(card);
      }
      // Inject nilai lama (name/nim)
      var nameInputs = wrap.querySelectorAll('input[name="names[]"]');
      var nimInputs  = wrap.querySelectorAll('input[name="nims[]"]');
      for (var i=0;i<nameInputs.length;i++){
        if (oldNames[i] !== undefined) nameInputs[i].value = oldNames[i] || '';
      }
      var oldNims = @json(old('nims', []));
      for (var j=0;j<nimInputs.length;j++){
        if (oldNims[j] !== undefined) nimInputs[j].value = oldNims[j] || '';
      }
      bindFileLabel(wrap);
      wireRemove(wrap);
      ...
      // default render berdasarkan type
      renderParticipantsForType(initialType);
    }
  })();
</script>
@endsection
